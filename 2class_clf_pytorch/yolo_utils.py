...
def draw_labels_and_boxes_1_stage(img, boxes, confidences, classids, idxs, colors, labels):
    # If there are any detections
    if len(idxs) > 0:
        idx_cnt = 0
        for i in idxs.flatten():
...
def generate_boxes_1_stage(outs, height, width, tconf , prob_thresh ):
    boxes = []
    confidences = []
    preds = []
    ...
    return boxes, confidences, preds
...
def infer_image(net, layer_names, height, width, img, colors, labels, FLAGS,
    ...
    if infer:
        ...
        blob = cv.dnn.blobFromImage(img, 1 / 255.0, (608, 608),
