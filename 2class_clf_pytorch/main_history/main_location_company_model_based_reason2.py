...
def predict_with_reason(
    ...
    for batch_dat in predict_loader:
        ...
        wComp = torch.ones_like(featComp)
        wLoc = torch.ones_like(featLoc)

        if use_cuda:
            wComp,wLoc = wComp.cuda(),wLoc.cuda()

        wComp.requires_grad, wLoc.requires_grad = True,True

        featComp = wComp*featComp
        featLoc = wLoc*featLoc

        model_output = model(feat_comp=featComp, feat_loc=featLoc, id_loc=featId, feat_ensemble_score=featEnsemble)
        outputs = model_output['outputs']

        targets = torch.ones_like(targets,dtype=targets.dtype)
        ...
        wComp = wComp - wComp.grad
        wLoc = wLoc - wLoc.grad
        x_grad = torch.cat([wComp,wLoc],dim=1)
        all_x_grads.append( x_grad )

        all_losses.append(loss.data.cpu().numpy())


    all_x_grads = torch.cat(all_x_grads)
    # torch.save(all_x_grads,'grad.pt')
    # all_x_grads.save('grad.pt')
