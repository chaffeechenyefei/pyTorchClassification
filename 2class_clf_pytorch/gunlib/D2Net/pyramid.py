...
from gunlib.D2Net.exception import EmptyTensorError
from gunlib.D2Net.utils import interpolate_dense_features, upscale_positions


def process_multiscale(image, model, scales=[.5, 1, 2]):
    b, _, h_init, w_init = image.size()
    device = image.device
    assert(b == 1)#only one image is tested each time
    ...
    for idx, scale in enumerate(scales):
        ...
        fmap_pos = torch.nonzero(detections[0]).t().cpu()#position(idx of all dims(here is 3) for non zero position)
        del detections
    ...
    del all_descriptors
    return keypoints, scores, descriptors#[Num,3] , [Num,] , [Num,512]
