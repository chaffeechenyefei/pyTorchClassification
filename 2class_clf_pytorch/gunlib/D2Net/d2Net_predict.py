...
class D2Net(nn.Module):
    def __init__(self, model_file=None, use_relu=True, use_cuda=True):
        ...
        if model_file is not None:
            self.load_state_dict(torch.load(model_file,map_location='cpu' if not use_cuda else 'cuda')['model'])#modified by Chaffee
...
class HardDetectionModule(nn.Module):
    ...
    def forward(self, batch):
        ...
        depth_wise_max = torch.max(batch, dim=1)[0] #@Chaffee It can be run because the batchsize is 1 in test mode. Otherwise, the code below should be used.
        ...
        # Some not said in the paper: Ref to SIFT for 'eliminating edge responses' for tricks here
        dii = F.conv2d(
...
#Ref to SIFT 'Accurate keypoint localization' for detail
