...
def predict_with_reason(
        model: nn.Module, criterion, predict_loader, use_cuda, test_pair, save_name: str, pre_name: str = '',
        lossType='softmax', sampling=True) -> Dict[str, float]:
    topk = 300
    model.train()
    model.freeze()
    ...
    for batch_dat in predict_loader:
        ...
        all_x_grads.append( F.normalize(x_grad,p=2,dim=1) )
    ...
    all_predictions = F.softmax(all_predictions, dim=1)
    all_predictions = all_predictions[:, 1].data.cpu().numpy()
    all_x_grads = all_x_grads.data.cpu().numpy()

    print('saving...')
    dat_pred_pd = pd.DataFrame(data=all_predictions.reshape(-1,1), columns=['similarity'])
    res_pd = pd.concat([test_pair, dat_pred_pd], axis=1)
